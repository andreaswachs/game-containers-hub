...
- Architectury API, Accessories, owo-lib, Lithostitched, Chipped, Forge Config API Port

# Client Requirements

Players need to install the same mods on their client to connect. The easiest way is to create a Modrinth modpack with all the mods listed in `versions.env` or share the `mods/` directory from the server.
